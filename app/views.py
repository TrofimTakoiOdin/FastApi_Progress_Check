...
from app.db_model import database as db
...
@app.get("/users/", response_model=List[models.ReadUser])
async def read_users():
    query = db.users.select()
    return await db.database.fetch_all(query)


@app.get("/products/", response_model=List[models.ReadProduct])
async def read_products():
    query = db.products.select()
    return await db.database.fetch_all(query)


@app.get("/orders/", response_model=List[models.ReadOrder])
...
@app.get("/users/{user_id}", response_model=models.ReadUser)
...
@app.get("/products/{product_id}", response_model=models.ReadProduct)
...
@app.get("/orders/{order_id}", response_model=models.ReadOrder)
...
@app.put("/users/{user_id}", response_model=models.ReadUser)
async def update_user(user_id: int, new_user: models.CreateUser):
    query = db.users.update().where(db.users.c.id == user_id).values(**new_user.dict())
    await db.database.execute(query)
    return {**new_user.dict(), "id": user_id}


@app.put("/products/{product_id}", response_model=models.ReadProduct)
async def update_product(product_id: int, new_product: models.CreateProduct):
    query = db.products.update().where(db.products.c.id == product_id).values(**new_product.dict())
    await db.database.execute(query)
    return {**new_product.dict(), "id": product_id}


@app.put("/orders/{order_id}", response_model=models.ReadOrder)
async def update_order(order_id: int, new_order: models.CreateOrder):
